...
class AdminUser extends Component {
  ...
  getNavData = async (id) => {
    ...
    const storeId = id || firstList.storeId;
    try {
      await UserActions.getUserList(storeId);
      const loggedInfo = this.props.result.toJS();
      const custNo = loggedInfo.firstUser ? loggedInfo.firstUser._id : '0';
  ...
  getRowData = async (id) => {
    const { UserActions, firstList } = this.props;
    const firstUserId = firstList.userId ? firstList.userId : '0';
    const custNo = id || firstUserId;
    ...
    return (
      <PageTemplate navData={store} id={currentStoreId} clickNav={this.getNavData}>
        ...
        <TableWithScroll
          ...
          button={
            currentCustNo && currentCustNo !== '0' ? (
              <TableBtn onClick={this.popDelete}>회원삭제</TableBtn>
            ) : null
          }
        />
        ...
        <PopDeleteConfirm
          ...
          getNavData={this.getNavData}
          closeDeletePop={this.closeDeletePop}
        />
      </PageTemplate>
    );
  }
